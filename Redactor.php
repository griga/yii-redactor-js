...
class Redactor extends CInputWidget
{
    ...
    public function run()
    {
        ...
        $options = array_merge($this->editorOptions, array(
//            'lang' => $this->lang, 'toolbar' => $this->toolbar,
            'plugins' => array('advanced')
...
JS;
        ...
        $css = <<<CSS
       .redactor_toolbar a.redactor_btn.redactor_btn_button_custom_1{
            background-image: url(/images/icons/button1.png);
       }
CSS;

        $cs->registerCss('redactor_cuatom_buttons', $css);
        $js = <<<JS
        if (typeof RedactorPlugins === 'undefined') var RedactorPlugins = {};
        RedactorPlugins.advanced = {
            init: function(){
                this.buttonAdd( 'button_custom_1','Вставить блок слева', function(){
                    this.insertHtmlAdvanced('<div>Привет</div>')
                });
                this.buttonAddSeparatorBefore('button_custom_1');
            }
        }
JS;
        // Register js code
        cs()->registerScript('RedactorAdvancedPlugin', $js, CClientScript::POS_HEAD);
    }
